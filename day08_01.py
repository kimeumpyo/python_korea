...
클래스 : 변수와 함수를 묶는 기술 (사용자에게 쉽게 제공하고 가독성을 좋게하기 위해서)
    ...
    메서드 - 클래스 안에서 정의되 있는 함수 (self를 갖고있음)

    객체화 - 클래스를 사용하기 위해선 변수에 담아 사용한다 이때 변수는 객체라고 부른다
    생성자 - 객체화하는 순산에 사용되는 메서드 def__init__(self):
    상속 - 클래스를 복붙
        복붙한 다음에 메서드를 추가하거나 재정의
'''
class MyClass:              # class 첫 문자는 대문자로 하는것이 일반적이다 
    멤버변수1 = ''
    멤버변수2 = 0
    멤버변수3 = []
    def 메서드1(self):
